...
create_fake_subjectDB <- function(n_subjects = 100, 
  ...
  all_s_stays = lapply(sIDs, function(sID) {
    ...
    for (fID in facilities) {
        stay = create_subject_stay(sID = sID, fID = fID, 
    ...
    if (with_errors) {
        ...
        all_s_stays = rbind(all_s_stays,
                            data.table(
                                sID = c(rep("X", 3), rep("Y", 4)),
                                fID = c("a", "b", "a", "a", "b", "c", "c"),
                                Adate = lubridate::as_datetime(c(Sys.Date(),
                                          Sys.Date() + 1,
                                          Sys.Date() + 9,
                                          Sys.Date(),
                                          Sys.Date() + 1,
                                          Sys.Date() + 8,
                                          Sys.Date() + 9)),
                                Ddate = lubridate::as_datetime(c(Sys.Date() + 7,
                                          Sys.Date() + 9,
                                          Sys.Date() + 10,
                                          Sys.Date() + 7,
                                          Sys.Date() + 4,
                                            ...
                                            n_clusters = 3)
{
  #create sub subjectDB
    ...
    s_stays = lapply(1:n_clusters, function(i) {
        create_fake_subjectDB(n_subjects = n_subjects_cl,
  ...
  connecting_stays = lapply(1:connecting_subjects[,.N], function(i){
    ...
    create_subject_stay(sID = connecting_subjects[i , sID], 
                        fID = connecting_f, 
